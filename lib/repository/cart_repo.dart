...
import 'package:demo_app/models/cart_model.dart'; // Assuming you have a CartModel class
...
class CartRepository {
  ...
  Future<void> addCartItem(Product product, {String? selectedSize}) async {
    ...
    if (index != -1) {
      // If item exists with the same size, update its quantity
      cartItems[index] = cartItems[index].copyWith(
        quantity: cartItems[index].quantity + 1,
      );
    } else {
      // If item does not exist or has a different size, add it
      cartItems.add(
